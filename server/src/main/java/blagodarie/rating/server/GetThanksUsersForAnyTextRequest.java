...
public class GetThanksUsersForAnyTextRequest
        extends ServerApiRequest<GetThanksUsersResponse> {

    private static final String TAG = GetThanksUsersForAnyTextRequest.class.getSimpleName();

    public GetThanksUsersForAnyTextRequest (
            @NonNull final String anyText,
            final int from,
            final int count
    ) {
        super(String.format(Locale.ENGLISH, "/getthanksusersforanytext?text=%s&from=%d&count=%d", anyText, from, count));
    }
